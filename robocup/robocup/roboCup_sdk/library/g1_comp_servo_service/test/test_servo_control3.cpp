...
int main(int argc, char** argv)
{
    ...
    Eigen::VectorXf target_angle_1(2);
    target_angle_1<<0,30;
    ...
    while(1)
    {
        ...
        if (_percent_1 < 1.0)
        {
            std::cout<<"debug _percent_1: "<<_percent_1<<std::endl;
            for (int id = 0; id < 2; id++)
            {
                target_angle_command(id) = (1 - _percent_1) * angle_init(id) + _percent_1 * target_angle_1(id);
                std::cout<<"debug target_angle_command(id: )  "<<id<<"  "<<target_angle_command(id)<<std::endl;
        ...
        {
            counter++;
            int desired0_position =  amplitude * sin(2 * M_PI * frequency * counter / 200.0);
            ...
            int desired1_position =  20 * sin(2 * M_PI * frequency * counter / 200.0)+target_angle_1(1);
            float joint1_servo_positon;
