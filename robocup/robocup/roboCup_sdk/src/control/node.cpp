...
// 1. 追球节点
BT::NodeStatus GoalieTrackBall::tick()
{
    ...
    // x轴活动范围
    const double goalie_x_behind = 4.3; // 球门线
    const double goalie_x_front = 2.0;  // 最远活动点
    const double ball_x_behind = 4.3;
    const double ball_x_front = 2.0;
    double ball_x = _interface->ballPositionInField(0);
    double ball_y = _interface->ballPositionInField(1);

    double t = (ball_x - ball_x_behind) / (ball_x_front - ball_x_behind);
    t = std::clamp(t, 0.0, 1.0);
    double goalie_x = goalie_x_behind + t * (goalie_x_front - goalie_x_behind);
    double goalie_y = std::clamp(ball_y, -1.0, 1.0);

    double robot_x = _interface->homoMatPelvisToField(0,2);
    double robot_y = _interface->homoMatPelvisToField(1,2);

    double dx = goalie_x - robot_x;
    double dy = goalie_y - robot_y;

    double kp = 0.8;
    double vx = kp * dx;
    double vy = kp * dy;

    vx = saturation(vx, Vec2<double>(-0.8, 0.8));
    vy = saturation(vy, Vec2<double>(-0.8, 0.8));

    // 朝向球
    double targetYaw = atan2(ball_y - robot_y, ball_x - robot_x);
    double vyaw = targetYaw;
...
// 2. 踢球节点
BT::NodeStatus GoalieKickBall::tick()
{
    if (!_interface->ballDetected) {
        return BT::NodeStatus::SUCCESS;
    }
    if(_interface->ballPositionInField(0) <= 1.8) {
        return BT::NodeStatus::SUCCESS;
    }
    double robot_x = _interface->homoMatPelvisToField(0,2);
    double robot_y = _interface->homoMatPelvisToField(1,2);
    double ball_x = _interface->ballPositionInField(0);
    double ball_y = _interface->ballPositionInField(1);
    double dist = std::hypot(ball_x - robot_x, ball_y - robot_y);
    ...
    if (dist < 0.25) {
        ...
        vx = saturation(vx, Vec2<double>(-1,1));
        vy = saturation(vy, Vec2<double>(-1,1));
...
// 3. 回家节点
BT::NodeStatus GoalieBackToHome::tick()
{
    // 计算机器人骨盆坐标系中的场中心位置
    Eigen::Vector2d homePositionInPelvis = dehomoVec(_interface->homoMatPelvisToField.inverse() * homoVec(Eigen::Vector2d(4.2, 0)));
    ...
    // 使用PD控制器计算速度
    const double kp = 0.65;  // 比例增益
    const double maxSpeed = 1;  // 最大速度
...
// 4. 找球节点
BT::NodeStatus GoalieFindBall::tick()
{
    if (!_interface->ballDetected)
    {
        if (firstRun)
        ...
        if(!firstRun){
            ...
            double y_pos = _interface->homoMatPelvisToField(1,2);
            const double y_min = -1.0;
            const double y_max = 1.0;
            const double speed = 0.8;
            if (y_pos >= y_max) direction = -1;
            if (y_pos <= y_min) direction = 1;

            _interface->locoClient.Move(0, speed * direction, 0);
        }
    }
    else
    {
...
// 5. 决策节点
BT::NodeStatus GoalieDecision::tick()
{
    std::string decision;
    ...
    if (_interface->ballDetected) {
        ...
        if (dist < 0.25) {
            decision = "GoalieKickBall";
        } else if (ball_x > 2 && fabs(ball_y) < 1.2) {
            decision = "robotTrackPelvis";
        } else {
            decision = "GoalieBackToHome";
        }
        } else {
            decision = "GoalieFindBall";
    }
...
{
    // 计算机器人骨盆坐标系中的场中心位置
    Eigen::Vector2d homePositionInPelvis = dehomoVec(_interface->homoMatPelvisToField.inverse() * homoVec(Eigen::Vector2d(1.3, 0)));
...
BT::NodeStatus Adjust::tick()
{
    ...
    double robotBallAngleToField = atan2(data->ball.posToField.y - data->robotPoseToField.y, data->ball.posToField.x - data->robotPoseToField.x);
...
BT::NodeStatus robotTrackField::tick()
{
    ...
    // Reduce the orbiting angle by decreasing the s parameter
    double s = 0.4;  // Changed from 0.4 to 0.2 to reduce orbiting angle
    double r = 0.8;
...
BT::NodeStatus kick::tick()
{
    ...
    Vec2<double> vecBallLeftGoalField;  //ball->left Goal quanjuweizhi
    ...
    {   
        ...
        double vx =0, vy =0, vtheta = 0;
        double s = 0.4, r=0.8;
        double ball_yaw =atan2(_interface->ballPositionInPelvis(1),_interface->ballPositionInPelvis(0));
        vx = s*sin(ball_yaw);
        vy = -s*cos(ball_yaw);
        vtheta = (ball_yaw+s)/r;
    ...
    double vyaw = _interface->ballYawToPelvis;
    vyaw = saturation(vyaw, Vec2<double>(-1,1));
...
{
    std::string decision;
    if(_interface->ballPositionInField(0)> 4.5||abs(_interface->ballPositionInField(1))>3) // Here, we simply treat the situation where the ball’s x-direction distance in the Field coordinate system is greater than 4.5 as a goal signal.
    {
         goalSignal = true;
        //goalSignal = false; 
    }
    ...
    double biasAngle = angleBallGoalField - anglePlevisBallfield;
    if(goalSignal|| ((abs(_interface->ballYawToPelvis)<=0.20) && (abs(_interface->ballPositionInPelvis(0)) <= 0.55) && (abs(_interface->ballPositionInPelvis(1)) <= 0.3)))
    {
