...
void G1Brain::updateBallMemory() {
    ...
    else
    {
        ...
        data->ball.posToField = dehomoVec(data->homoMatPelvisToField * homoVec(data->ballPositionInPelvis));
        ...
        data->ball.pitchToRobot = asin(config->height / data->ball_);
    } 
}
