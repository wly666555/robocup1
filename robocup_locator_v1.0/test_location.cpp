...
int main(int argc, char *argv[])
{
    ...
    // Create wrist Subscriber：lowState是一个订阅者对象，用于接收来自"rt/lowstate"主题的LowState消息。
    ...
    // 该主题包含了机器人的里程计状态信息(例如位置、姿态)，odomState对象通过订阅该主题来获取这些信息。
    ...
    while (true) {// 主循环，持续运行定位任务
        ...
        locator.robotPoseToOdom.theta = odomState->msg_.imu_state().rpy()[2]; // 从里程计状态消息中读取机器人的姿态数据。

        std::cout << "Odometer information: (" // 输出机器人的位置和姿态信息
                    << locator.robotPoseToOdom.x << ", "
                    << locator.robotPoseToOdom.y << ", "
                    << locator.robotPoseToOdom.theta << ")" << std::endl;

        // Read servo data
        double wrist_yaw_angle = rad2deg(lowState->msg_.motor_state()[JointIndex::kWaistYaw].q()); // 从lowstate中读取手腕偏航角度，并转换为度数。
