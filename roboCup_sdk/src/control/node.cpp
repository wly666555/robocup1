...
BT::NodeStatus GoalieBackToHome::tick()
{
    ...
    {
    // 计算机器人骨盆坐标系中的场中心位置
        Eigen::Vector2d homePositionInPelvis = dehomoVec(_interface->homoMatPelvisToField.inverse() * homoVec(Eigen::Vector2d(-3.5, 0)));
        // 设置目标位置容差
        ...
        if(_interface->ballPositionInPelvis[0] < -1.2 && _interface->ballPositionInPelvis[1] < 2.3)
        {
            break;
            return BT::NodeStatus::SUCCESS;
        }
...
BT::NodeStatus GoalieDecision::tick()
{
    ...
    // 判断是否可以踢球
    double ball_x1 = _interface->ballPositionInField[0];
    double ball_y1 = _interface->ballPositionInField[1];
    double ball_x = _interface->ballPositionInPelvis[0];
    double ball_y = _interface->ballPositionInPelvis[1];
    double dist = sqrt(ball_x * ball_x + ball_y * ball_y);
    if(_interface->ballDetected)
    {
        if (dist < 0.5) {
        decision = "GoalieKickBall";
        }
        else if ((ball_x1 < -1.2 && fabs(ball_y1) < 2.3) ) {
            decision = "GoalieTrackBall";
        } 
        else if(_interface->homoMatPelvisToField(0,2) > -1)
        {
    ...
    std::cout << "ballDetected: " << _interface->ballDetected << std::endl;
    std::cout << "ball_x1: " << ball_x1 << ", ball_y1: " << ball_y1 << std::endl;
...
{
    // 计算机器人骨盆坐标系中的场中心位置
    Eigen::Vector2d homePositionInPelvis = dehomoVec(_interface->homoMatPelvisToField.inverse() * homoVec(Eigen::Vector2d(-1.5, 0)));
