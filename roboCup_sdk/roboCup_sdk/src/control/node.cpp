...
{
    std::string decision;
    if(_interface->ballPositionInField(0)> 4.6||abs(_interface->ballPositionInField(1))>3) // Here, we simply treat the situation where the ball’s x-direction distance in the Field coordinate system is greater than 4.5 as a goal signal.
    {
         goalSignal = true;
        //goalSignal = false; 
    }
